...
void move() {
	int a; //달력이동 변수
	mainmemu();
	a = _getch(); // [입력하면 엔터 없이 바로 이동] = system("pause"); 키 입력 받으면 이동 
	switch (a) {
	case 67: calendar(); break;// C 아스키 코드
	case 68: submemu(); break; // D 아스키 코드
	case 99: calendar(); break; // c 아스키 코드
	case 100: submemu(); break; // d 아스키 코드
	default: break;
	}
}
